...
<body>
    <div class="content">
        ...
        <div class="content">
            <img style="padding-bottom: 4px;" src="{{ asset('images/logo_pronova.jpg') }}" width="120" alt="Logo "><br>
            ...
            @if ($serviceRequest->SignatureFlows->count() == 0)
                <div class="nueve">
                    <div class="justify" style="width: 100%;">
                        De conformidad a lo dispuesto en la Ley N° 21.192, de 2019 del Ministerio de Hacienda, que
                        aprueba el presupuesto del Sector Público para el año 2020, vengo en certificar que la Dirección
                        del Servicio de Salud Iquique
                        cuenta con el Presupuesto para contratación de don (ña) <b><span
                                class="uppercase">{{ $serviceRequest->employee->fullName }}</span></b>
                        desde el <b>{{ $serviceRequest->start_date->format('d-m-Y') }} hasta el
                            {{ $serviceRequest->end_date->format('d-m-Y') }}</b>
                        por contingencia originada por la Pandemia Coronavirus (COVID-19), con cargo al SUBTITULO
                        21-03-001-001-02, personal no médico,
            ...
            @else
                <div class="nueve">
                    <div class="justify" style="width: 100%;">
                        ...
                        por contingencia originada por la Pandemia Coronavirus (COVID-19), con cargo al SUBTITULO
                        21-03-001-001-02, personal no médico,
                        <u>"Honorario Suma Alzada en el Hospital General"</u>
                        <!-- Mediante el presente certifico que don(a) o Sr(a) <b><span class="uppercase">{{ $serviceRequest->employee->fullName }}</span></b> ha desempeñado las actividades estipuladas
          en su convenio de prestación de servicios con el Hospital Dr.Ernesto Torres Galdames durante el preríodo de contingencia COVID
          del <b>{{ $serviceRequest->start_date->format('d/m/Y') }}</b> al <b>{{ $serviceRequest->end_date->format('d/m/Y') }}</b>,
          registrando @if ($serviceRequest->type == 'Turnos')
los siguientes turnos extra:
@else
los siguientes turnos extra:
@endif -->
                    </div>
...
@endforeach
        ...
        <span class="uppercase">{{ $serviceRequest->SignatureFlows->where('sign_position', 2)->first()->user->organizationalUnit->name }}</span><br> -->
                        <span
                            class="uppercase">{{ $serviceRequest->SignatureFlows->where('sign_position', 2)->first()->user->getFullNameAttribute() }}</span><br>
                        <!-- <span class="uppercase">{{ $serviceRequest->SignatureFlows->where('sign_position', 2)->first()->user->position }}</span><br>
        <span class="uppercase">{{ $serviceRequest->SignatureFlows->where('sign_position', 2)->first()->user->organizationalUnit->name }}</span><br> -->
                        <!-- 1 Hospital -->
