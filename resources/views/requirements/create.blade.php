        ...
        @if ($parte->id != 0)
            ...
            @if ($parte->files != null)
                @foreach ($parte->files as $file)
                    <a href="{{ route('documents.partes.download', $file->id) }}" target="_blank" data-toggle="tooltip"
                        data-placement="top" data-original-title="{{ $file->name }}">
                    ...
                    <fieldset class="form-group @if ($parte->id != 0) col-12 @else col-5 @endif">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <button type="button" class="btn btn-outline-primary add-destinatario" data-toggle="tooltip"
                                    data-placement="top" title="Utilizar para agregar más de un destinatario">
            ...
            <table id="tabla_funcionarios" class="table table-striped table-sm" style="display: none">
                ...
                <div class="form-row">
                    <fieldset class="form-group @if ($parte->id != 0) col-12 @else col-6 @endif">
                        ...
                        <input type="text" class="form-control" id="for_subject" name="subject" required="required"
                            value="{{ $parte->subject }}">
                    </fieldset>
                    ...
                    <fieldset class="form-group @if ($parte->id != 0) col-12 @else col-3 @endif">
                        ...
                        <input type="datetime-local" class="form-control" id="for_limit_at" name="limit_at">
                    </fieldset>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".add-destinatario").click(function() {
                ...
                if (array) {
                    for (var i = 0; i < array.length; i += 1) {
                        if ($("#user option:selected").val() == array[i]) {
                            alert("Ya se ingresó este usuario");
                            return;
                        }
                    }
                }
                ...
                array.push(user_id);
                var markup = "<tr id='row_to" + user_id +
                    "'><input type='hidden' id='users' name='users[]' value=" + user_id + ">" +
                    "<input type='hidden' name='enCopia[]' value='0'>" +
                    "<td>" + ou + "</td>" +
                    "<td>" + user + "</td>" +
                    "<td><input type='checkbox' disabled></td>" +
                    "<td><button type='button' onclick = 'deleteRow(" + user_id + "," + array +
                    ")' class='btn btn-outline-secondary btn-sm'><span class='fas fa-trash-alt' aria-hidden='true'></span></button></td>" +
                    "</tr>";
            ...
            $(".add-destinatario-cc").click(function() {
                ...
                $("#tabla_funcionarios").fadeIn(1000);
                var ou = $("#ou option:selected").text();
                var user = $("#user option:selected").text();
                var user_id = $("#user option:selected").val();
                ...
                var markup = "<tr id='row_to" + user_id +
                    ...
                    "<td><button type='button' onclick = 'deleteRow(" + user_id +
                    ")' class='btn btn-outline-secondary btn-sm btn-delete'><span class='fas fa-trash-alt' aria-hidden='true'></span></button></td>" +
                    "</tr>";
                $("#tabla_funcionarios").append(markup);
            });
        });
        ...
        $(".add-row").click(function() {
            ...
            if (flag == 1) {
                var markup = "<input type='hidden' name='documents[]' value=" + doc_id + ">" +
                    "<td align='left'>" + doc_id + "-" + doc_subject + "<br> </td>";
                $("#tabla_documents").append(markup);
                $("#for_document").val("");
                $("#for_document").focus();
            } else {
                alert("No existe documento, favor intente otro.");
            }
        ...
        function funcionDeferred() {
            ...
            $.get('{{ route('rrhh.users.get.from.ou') }}/' + ou_id, function(data) {
                ...
                $.each(data, function(index, subCatObj) {
                    $('#user').append('<option value="' + subCatObj.id + '">' + subCatObj.name + ' ' +
                        subCatObj.fathers_family + ' ' + subCatObj.mothers_family + '</option>');
                });
                deferred.resolve();
            });
        ...
        //funcion para eliminar celda de tabla de funcionarios
        function deleteRow(id, array_actual) {
            array_actual = $.grep(array_actual, function(n) {
                return n != id;
            });
            $("#row_to" + id).remove();
            array = array_actual;
