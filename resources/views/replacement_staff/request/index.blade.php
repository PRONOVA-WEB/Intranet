            ...
            <tbody class="small">
                @foreach ($pending_requests as $requestReplacementStaff)
                    <tr>
                                ...
                                @if ($sign->request_status == 'accepted')
                                    <span style="color: green;">
                                        <i class="fas fa-check-circle fa-2x"
                                            title="{{ $sign->organizationalUnit->name }}"></i>
                                    </span>
                                @endif
                        ...
                        <td>
                            @if ($requestReplacementStaff->RequestSign->last()->request_status == 'accepted' && !$requestReplacementStaff->technicalEvaluation && Auth::user()->hasPermissionTo('Replacement Staff: assign request'))
