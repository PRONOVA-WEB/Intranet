          ...
          if ($this->program_contract_type == "Mensual") {
            ...
            if (Auth::user()->organizationalUnit->establishment_id == 1) {
              ...
              if (Auth::user()->organizationalUnit->id == 24) {
                ...
                $this->signatureFlows['S.D.G.A'] = Authority::getAuthorityFromDate(2,now(),['manager'])->user->id; // 2 - Subdirección de Gestion Asistencial / Subdirección Médica
                ...
                $this->signatureFlows['S.G.D.P'] = Authority::getAuthorityFromDate(44,now(),['manager'])->user->id; // 44 - Subdirección de Gestión y Desarrollo de las Personas
                $this->signatureFlows['S.D.A'] = Authority::getAuthorityFromDate(40,now(),['manager'])->user->id; // 31 - Subdirección de Recursos Físicos y Financieros
                //SE COMENTA Y QUITA DEL FLUJO A PETICIÓN DE LA SUBDIRECTORA
                // $this->signatureFlows[Authority::getAuthorityFromDate(1,now(),['manager'])->position->name . " - " . Authority::getAuthorityFromDate(1,now(),['manager'])->user->organizationalUnit->establishment->name] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 1 - Dirección
              }
            ...
            //hospital
            // se comenta 2 y 44, ya que se solicitó quitar autoridades del de flujo de hospital
            elseif(Auth::user()->organizationalUnit->establishment_id == 1){
              ...
              // $this->signatureFlows['S.G.D.P'] = Authority::getAuthorityFromDate(44,now(),['manager'])->user->id; // 44 - Subdirección de Gestión y Desarrollo de las Personas
              $this->signatureFlows[Authority::getAuthorityFromDate(84,now(),['manager'])->position->name . " - " . Authority::getAuthorityFromDate(84,now(),['manager'])->user->organizationalUnit->establishment->name] = Authority::getAuthorityFromDate(84,now(),['manager'])->user->id; // 84 - Dirección
            }
            ...
            //hospital
            elseif(Auth::user()->organizationalUnit->establishment_id == 1){
              $this->signatureFlows['Subdirector'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 88 - Subdirección Médica - 9882506 - (iriondo)
              $this->signatureFlows['S.G.D.P Hospital'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 86 - Subdirección de Gestión de Desarrollo de las Personas
              $this->signatureFlows['Jefe Finanzas'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 11 - Departamento de Finanzas
            }
          }
        }else{
          if (Auth::user()->organizationalUnit->establishment_id == 1) {
            $this->a = "suma";
            $this->signatureFlows['S.G.D.P Hospital'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id;
            $this->signatureFlows['Jefe Finanzas'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id;
            $this->signatureFlows[Authority::getAuthorityFromDate(1,now(),['manager'])->position->name . " - " . Authority::getAuthorityFromDate(1,now(),['manager'])->user->organizationalUnit->establishment->name] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id;
          }else{
            $this->signatureFlows['Planificación CG RRHH'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 59 - Planificación y Control de Gestión de Recursos Humanos
            $this->signatureFlows['S.G.D.P'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 44 - Subdirección de Gestión y Desarrollo de las Personas
            $this->signatureFlows['S.D.A'] = Authority::getAuthorityFromDate(1,now(),['manager'])->user->id; // 31 - Subdirección de Recursos Físicos y Financieros
          }
        }
