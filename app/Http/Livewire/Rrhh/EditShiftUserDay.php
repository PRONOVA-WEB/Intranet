...
class EditShiftUserDay extends Component
{
	...
	public function findAvailableExternalDaysToChange(){
        $monthYearFilterArray  = explode('-',$this->monthYearFilter);
		$this->dayToToChange = 0;
		$this->availableExternalDaysToChange= array();

        $actuallyMonth = $monthYearFilterArray[1];
        $actuallyYear = $monthYearFilterArray[0];
	...
	public function update(){//funcion que actualiza la informacion segun el estado elegido del select accion en el modal
		if( ($this->action != 1 && $this->action != 7 && $this->action != 14 && $this->action !=  16) &&  isset($this->shiftUserDay) ){ // este if es para filtrar solo los que son cambio de estado
			...
			if($this->userIdtoChange != 0){ // si el id es ditinto a 0 = dejar dia laboral disponible, ahora elijo un dia en especifico para intercambiar
        		...
        		if($this->dayToToChange == 0 ){ //osea NO  intercambiar por ningun dia, sino solo traspasar el dia
					...
					$bTurno = ShiftUser::where("user_id",$this->userIdtoChange)
                                        ->where("date_from",">=",$from)
                                        ->where("date_up","<=",$to)
                                        ->where('shift_types_id',$this->shiftUserDay->ShiftUser->shift_types_id)
                                        ->first();

                    if( !isset($bTurno) ){ // si no tiene ningun turno asociado a ese rango, se le crea
						$bTurno = new ShiftUser;
						$bTurno->date_from = $from;
						$bTurno->date_up = $to;
						$bTurno->asigned_by = Auth::user()->id;
		...
		return redirect('/rrhh/shift-management?orgunitFilter='.$this->shiftUserDay->ShiftUser->user->organizationalUnit->id.'&turnFilter='.$this->shiftUserDay->ShiftUser->shift_types_id.'&monthYearFilter='.$this->monthYearFilter.'');
	}
	public function confirmExtraDay(){

		 return redirect()->route('rrhh.shiftManag.confirmDay',['day'=>$this->shiftUserDay,'monthYearFilter'=>$this->monthYearFilter]);
	}
  ...
  //   	$this->dayToToChange = $this->dayToToChange;
    ...
    {
        return view('livewire.rrhh.edit-shift-user-day',["tiposJornada"=>$this->tiposJornada,"estados"=>$this->estados,"statusColors"=>$this->colors]);
    }
}
